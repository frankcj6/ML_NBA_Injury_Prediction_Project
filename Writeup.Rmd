...
##Data collection
  ...
  We scrape tables from Basketball reference NBA Schedule and Results of 2016-2017 and 2017-2018(i.e. *https://www.basketball-reference.com/leagues/NBA_2018_games-october.html* for 2017-2018) and concat them to get our game-level data. We also add a column to indicate the distance from the home city. This game-level data is used to summarise game results and the number of the audience attended, and to disentangle their relationship. We get our player-level data by scraping and concating tables from Basketball reference NBA Scores & Boxes for each season of the two years (i.e. *https://www.basketball-reference.com/boxscores/201910220LAC.html* for Oct, 2019). For each match, there are originally four tables: 2 teams (Home/Away) * 2 Box Scores Stats (Basic/Advanced). For analysis purpose, we combine the four tables into one by adding a Home/Away flag and joining Basic and Advanced Box Scores Stats by players. We also add a column to indicate whether the player is a starter or a reserve. In this way, we can get one table with a distinct player row for each game. To avoid the self-correlation between win and loss of different teams that are playing against each other, we treat each game results independently to solve the issue of 'game theory'. 
  For our injury model and prediction, we parsed data from *https://www.prosportstransactions.com/basketball/Search/Search.php* for exploratory research. We download the player-level injury data from  *https://www.kaggle.com/ghopkins/nba-injuries-2010-2018/version/1* and join it with previous player-level data. Then we get the player injury data to make injury predicton.
